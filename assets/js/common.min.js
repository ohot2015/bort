$(document).ready(function(){
    $(".SlectBox").SumoSelect({showTitle:false});
    $('.SlectBox.first').on('sumo:closing', function() {
        xz._data.currency = $('.first + p span').html().trim() == 'WMZ'?2:1
    });
    $('.SlectBox.second').on('sumo:closing', function() {
        xz._data.currency2 = $('.second + p span').html().trim() == 'WMZ'?2:1
    });
    $('.SlectBox.first')[0].sumo.selectItem(0)
    $('.SlectBox.second')[0].sumo.selectItem(0);

});
"use strict";

Vue.filter('toCurrency', function (value) {
    if (typeof value !== "number") {
        return value;
    }

    var formatter = new Intl.NumberFormat('ru', {
        //                style: 'currency',
        //                currency: 'RUB',
        error:'',
        minimumFractionDigits: 0
    });
    return formatter.format(value);
});
var xz = new Vue({
    el: '#app',
    data: {
        data_landing: {},
        buy: '',
        sell: '',
        buy3: '',
        sell3: '',
        currency: 1,
        currency2: 1
    },
    computed: {
        buy2: function buy2() {
            var factor;

            if (this.currency == 1) {
                factor = this.data_landing.buy_WMR;
            } else if (this.currency == 2) {
                factor = this.data_landing.buy_WMZ;
            }

            return this.buy * factor;
        },
        sell2: function sell2() {
            var factor;

            if (this.currency2 == 1) {
                factor = this.data_landing.sale_WMR;
            } else if (this.currency2 == 2) {
                factor = this.data_landing.sale_WMZ;
            }

            return this.sell / factor;
        },
        cleanPhone: function cleanPhone() {
            if (this.data_landing.phone) {
                return 'tel:' + this.data_landing.phone.replace(/[-+()\s]/g, '');
            }

            return '';
        }
    },
    methods:{
        initHandler(){

        },
        inputBuy(e){

            var factor;

            if (this.currency == 1) {
                factor = this.data_landing.buy_WMR;
            } else if (this.currency == 2) {
                factor = this.data_landing.buy_WMZ;
            }

            this.$refs.buy.value = this.$options.filters.toCurrency(e.target.value / factor)

        },
        inputBuy2(e){

            var factor;

            if (this.currency2 == 1) {
                factor = this.data_landing.sale_WMR;
            } else if (this.currency2 == 2) {
                factor = this.data_landing.sale_WMZ;
            }

            this.$refs.buy2.value = this.$options.filters.toCurrency(e.target.value * factor)

        },
        wmid() {
            this.$refs.inp1.select();
            document.execCommand('copy');
        },
        wmz() {
            this.$refs.inp2.select();
            document.execCommand('copy');
        },
        wmr() {
            this.$refs.inp3.select();
            document.execCommand('copy');
        }
    },

    created: function created() {
        var _this = this;

        fetch("/ajax.php").then(function (res) {
            return res.json();
        }).then(function (res) {
            _this.data_landing = res;
        });

    }
});
