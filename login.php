...
if(isset($_POST['submit']))
{
    ...
    foreach ($data['users']  as $key => $user) {
        // Сравниваем пароли
        if ($user['user_login'] === $_POST['login'] && $user['user_password'] === $_POST['password']) {
            // Генерируем случайное число и шифруем его
            $hash = md5(generateCode(10));

            $data['users'][$key]['user_hash'] = $hash;
            $yaml = Yaml::dump($data);
            
            file_put_contents('config.yaml', $yaml);

            // Ставим куки
            setcookie("id", $key, time()+60*60*24*30);
        ...
        <div class="col-md-offset-3 col-md-6">
            <form class="form-horizontal" method="POST">
                ...
                <div class="form-group">
                    <input type="email" class="form-control" id="inputEmail" placeholder="E-mail" name="login">
